...
def import_chat_from_html():
    ...
    with st.expander("Import Chat from saved HTML file", expanded=True):
...
def import_chat_from_csv():
    ...
    with st.expander(f"""Import Chat from CSV file (columns={header})""", expanded=True):
        df_imp = None
        csv_data = None
        _, c1 = st.columns([6,3])
        with c1:
        ...
        if df_imp is not None:
            ...
            with DBConn() as _conn:
                ...
                df_imp.to_sql(CFG["CHAT_TABLE"], _conn, if_exists="append", index=False)
                # st.dataframe(df_imp)
def main():
